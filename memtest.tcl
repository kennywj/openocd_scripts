...
proc fillmem32 { addr len value } {
    for {set i 0} {$i < $len} {incr i 4} {
		memwrite32 [expr $addr + $i] $value
	}
}

proc fillmem16 { addr len value } {
    for {set i 0} {$i < $len} {incr i 2} {
		memwrite16 [expr $addr + $i] $value
	}
}

proc fillmem8 { addr len value } {
    for {set i 0} {$i < $len} {incr i 1} {
		memwrite8 [expr $addr + $i] $value
	}
}

proc cmpmem { src dst len } {
    for {set i 0} {$i < $len && $i >= 4} {incr i 4} {
        set addr [expr $src + $i]
    	set data0 [memread32 $addr]
    	set addr [expr $dst + $i]
    	set data1 [memread32 $addr]
		if {[expr $data0 != $data1]} {
			echo "FAILED compare memory: Address: [convertToHex $addr], [convertToHex $data0], [convertToHex $data1], offset: $i"
	    	return
		}
	}
	if {$i < $len} {
		for {} {$i < $len} {incr i 1} {
			set addr [expr $src + $i]
    		set data0 [memread8 $addr]
    		set addr [expr $dst + $i]
    		set data1 [memread8 $addr]
			if {[expr $data0 != $data1]} {
				echo "FAILED compare memory: Address: [convertToHex $addr], [convertToHex $data0], [convertToHex $data1], offset: $i"
	    		return
	    	}
	...
	echo "SUCCESS compare memory $src and $dst, size $len equal"
	return
}